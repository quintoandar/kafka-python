...
def step_impl_given_idempotent_key_defined(context):
    context.RedisIdempotenceClient.idempotent_key = lambda message: message.value


@when("Key exists in redis")
def step_impl_when_key_exists(context):
    context.RedisIdempotenceClient.redis.get = MagicMock(return_value={"key": 1})
...
def step_impl_when_mark_consumed_message_called(context):
    ...
    context.expected_key = "{}-{}-{}".format(
        context.message.topic, context.RedisIdempotenceClient.group_id, hash(str(key))
    )
...
def step_impl_when_is_unique_called(context):
    ...
    context.RedisIdempotenceClient.is_unique(context.message.topic, context.message)


@then("The correct key should be saved on redis")
def step_impl_then_verify_correct_set_params(context):
    context.RedisIdempotenceClient.redis.set.assert_called_with(
        context.expected_key, 1, ex=context.RedisIdempotenceClient.expire
    )


@then("The correct key should be passed to redis")
def step_impl_then_verify_correct_get_params(context):
    context.RedisIdempotenceClient.redis.get.assert_called_with(context.expected_key)


@then("is_unique should return true")
def step_impl_then_verify_true(context):
    assert_that(
        context.RedisIdempotenceClient.is_unique(message.topic, message) is True
    )
...
def step_impl_then_verify_false(context):
    assert_that(
        context.RedisIdempotenceClient.is_unique(message.topic, message) is False
    )
...
def step_impl_then_verify_no_errors(context):
    assert_that(
        context.RedisIdempotenceClient.mark_consumed_message(message.topic, message)
        is None
    )
